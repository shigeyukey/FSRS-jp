...
# 目次
- [超短縮版](#the-ultra-short-version)
- [ステップ1: FSRSを有効にする](#step-1-enable-fsrs)
- [ステップ2: FSRSの設定を構成する](#step-2-configure-fsrs-settings)
- [ステップ3: 最適なパラメータを見つける](#step-3-find-optimal-parameters)
- [ステップ4: (オプション) パラメータを評価する](#step-4-optional-evaluate-the-parameters)
- [ステップ5: (オプション) 最小推奨保持率を計算する](#step-5-optional-compute-minimum-recommended-retention)
- [ステップ6: (オプション) カスタムスケジューリング](#step-6-optional-custom-scheduling)
- [FAQ](#faq)
...
以下の図は、保持率に応じた作業量の変化を示しています。**曲線の正確な形状は、ユーザーのパラメータと学習習慣に依存します。** 最適な保持率を見つけるには、[ステップ5](#step-5-optional-compute-minimum-recommended-retention)で説明されている「最小推奨保持率を計算する（実験的）」機能を使用してください。
